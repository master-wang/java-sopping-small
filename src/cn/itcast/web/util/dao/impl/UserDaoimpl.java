...
public class UserDaoimpl implements UserDao {
    ...
    public int updateUserbyId(User user) {
        String sql ="update user set password = ? , `name` = ? , `phone` = ? , `sex` = ? , `age` = ? , `desc` = ?  , `is_admin` = ?  where id = ?";
        int count = template.update(sql,user.getPassword(),user.getName(),user.getPhone(),user.getSex(),user.getAge(),user.getDesc(),user.getIs_admin(),user.getId());
        System.out.println("更新用户信息："+count);
        return count;
    }
